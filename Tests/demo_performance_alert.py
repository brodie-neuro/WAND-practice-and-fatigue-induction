...
from wand_nback.performance_monitor import (
    MonitorConfig, check_sequential_block, handle_flag, reset_flag_count,
)
...
config = MonitorConfig(
    ...
    missed_response_threshold=0.20, action="warn_then_terminate",
)
...
info.draw(); win.flip(); core.wait(2.0)

# Simulating 45 lapses out of 162 trials (27% lapse rate)
check3 = check_sequential_block(
    {"Overall D-Prime": 1.50, "Correct Responses": 104, "Incorrect Responses": 13, "Lapses": 45},
    block_number=3, config=config,
)
...
info2.draw(); win.flip(); core.wait(2.0)

check4 = check_sequential_block(
    {"Overall D-Prime": 0.44, "Correct Responses": 73, "Incorrect Responses": 54, "Lapses": 5},
    block_number=4, config=config,
)
