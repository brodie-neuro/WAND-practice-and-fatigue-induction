...
The induction stage combines Sequential, Spatial, and Dual N-back tasks. Sequential blocks run at the calibrated N-level and can include brief visual distractors. Spatial and Dual blocks adapt within each 4.5-minute block using three sub-blocks, with promotion at or above 82% accuracy and demotion at or below 65%, bounded to Levels 2-4. Spatial and Dual blocks also support progressive timing compression. Subjective ratings of fatigue, effort, attention, and overwhelm are inserted at configured points. Results are saved after each block and at session end, including accuracy, reaction time, lapses, d-prime, criterion ($c$), A-prime, hit rate, false-alarm rate, hits, misses, correct rejections, false alarms, and task metadata. A performance monitor can flag low Sequential d-prime or high lapse rates and can log, warn, or terminate according to configuration.
